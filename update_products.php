<?php
	...
	$subcategory = strtolower($js['subcategory']);
	...
	if(!empty($product_id) && !empty($name)){
		...
		//	->addImageToMediaGallery($prod_images, array('image','thumbnail','small_image'), true, false) //assigning image, thumb and small image to media gallery
			...
			->setCategoryIds($subcategory); //assign product to categories
			if($product->save()){
			...
			//endif;
				...
				//$msg = $e->getMessage();
				$json = array("error" => true, "message" => "Update Error!");
			}
	}else{
		
		$json = array("error" => true, "message" => "Product ID/Name blank!");
	}
