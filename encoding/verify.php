<?php
    ...
    if(!empty($seller_id) && !empty($tag_id) && !empty($product_id)){
				...
				if($product_id == $productDetail['product_id']){
					...
					if($cnt == 1){
						...
						if(distance($latitude, $longitude,$lat,$long,"M") < 500){
							...
							$query = "UPDATE `encode` SET `added_by` = 'seller', `bussiness_customer_id` = '".$seller_id."', `product_id` = '".$product_id."' , `chip_latitude` = '".$latitude."' , `chip_longitude` = '".$longitude."', `chip_address` = '".$pr_address."' WHERE  `tag_id` = '".$tag_id."'";
                        ...
                        $json = array("error" => true, "type" =>"2", "message" => "Please add GPS location");   						
		...
		$json = array("error" => true, "type" =>"2", "message" => "SellerID, TagID, ProductID not blank!");
	}
    /* Output header */
